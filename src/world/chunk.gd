...
public partial class Chunk : StaticBody3D
{
	...
	// ----- Used by threaded manager to inject blocks -----
	...
	// ----- Packing helpers (also used on main thread if needed) -----
	...
	// ----- Optional in-chunk data generation (single thread use) -----
	...
	// ----- Mesh generation (CPU only, no neighbour culling) -----
	...
	public void GenerateMeshData()
	{
		...
		foreach (uint packed in _blocks)
		{
			...
			// Add all 6 faces (no neighbour culling; simpler & safe across chunks)
	...
	// ----- If you want to draw a single chunk (not required for Region) -----

	public void ApplyMesh()
	{
		if (_verticesArray == null || _verticesArray.Length == 0)
			return;

		if (_surfaceArray == null)
			_surfaceArray = new Godot.Collections.Array();

		if (_surfaceArray.Count < (int)Mesh.ArrayType.Max)
			_surfaceArray.Resize((int)Mesh.ArrayType.Max);

		if (_normalsArray == null || _normalsArray.Length != _verticesArray.Length)
			_normalsArray = new Vector3[_verticesArray.Length];

		if (_colorsArray == null || _colorsArray.Length != _verticesArray.Length)
			_colorsArray = new Color[_verticesArray.Length];

		_surfaceArray[(int)Mesh.ArrayType.Vertex] = _verticesArray;
		_surfaceArray[(int)Mesh.ArrayType.Normal] = _normalsArray;
		_surfaceArray[(int)Mesh.ArrayType.Color]  = _colorsArray;

		var arrMesh = MeshInstance.Mesh as ArrayMesh;
		if (arrMesh == null)
		{
		...
		arrMesh.AddSurfaceFromArrays(Mesh.PrimitiveType.Triangles, _surfaceArray); // [web:123]

		if (arrMesh.GetSurfaceCount() > 0 && Material != null)
